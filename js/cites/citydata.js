var cities=[
  ...
  var rc=cities;
  var distState=new Set();
  //console.log(distinctCS(rc));
  //console.log(listCitiesByState());
  //sameLatCitiesWithInStates();
  function distinctCS(cit){
    cit.forEach(element => {
      distState.add(element.admin_name);
    });
    return distState;
  }
  function listCitiesByState() {
    let mc=new Map();
    distinctCS(rc).forEach((element)=> {mc.set(element,valueC(element))});
      return mc;
  }
  function valueC(e){
    var ans=[];
     cities.forEach(ele => {
      if(ele.admin_name==e){ans.push(ele.city)}
    });
    return ans;
  }
  function sameLatCitiesWithInStates() {
    let m1=new Map();
    distinctCS(rc).forEach((element)=> {m1.set(element,valueM1(element))});
    console.log(m1);
  }
  function valueM1(e) {
    var setLat=new Set();
    ...
    cities.forEach(element => {
      setLat.add(parseInt(element.lat));
    });
    setLat.forEach(ele => {
      {sa.push({lat :ele,city :valueLC(ele)})}
    });
    return sa;
  }
  function valueLC(e) {
    var ans=[];
     cities.forEach(ele => {
      if(parseInt(ele.lat)==e){ans.push(ele.city)}
    });
    console.log(ans);
    return ans;
  }

  result = cities.slice(0,10).reduce( (r, a)=> {

    r[a.admin_name] = r[a.admin_name] || [];

    r[a.admin_name].push(a.city);
...
var x=cities.sort((a, b) => parseFloat(b.population) - parseFloat(a.population)).map(x=>x.city+x.population);

console.log(x);



var counts = cities.reduce((p, c) => {

    var name = c.admin_name;
